...
export default class BlastsTrap extends Phaser.GameObjects.Sprite {
    ...
    public wall : Phaser.Tilemaps.TilemapLayer;
  ...
  CreateBlasts(){
      ...
      if(distanceX <= 150 && distanceY <= 150){
          ...
          const blast2 = new Blasts(this.scene , this.x , this.y)
          ...
          const blast3 = new Blasts(this.scene , this.x , this.y)
        ...
        this.scene.physics.add.collider(blasts , this.wall , blast.DestroyBlasts)
        /* Sets the player's damage
        this.scene.physics.add.collider(this.player , blasts , blast.Damageplayer)
        */
      }
      else{
          this.play("trap-idle")
